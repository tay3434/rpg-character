...
const [provideAsync, getProvideAsync, initProvideAsync] = createMark(({ name, kind }) => {
    if (kind === 'setter')
        throw new Error(`@provide cannot decorate setter ${String(name)}`);
    if (kind === 'method')
        throw new Error(`@provide cannot decorate method ${String(name)}`);
    ...
    constructor(...args) {
        ...
        if (provides.size || providesAsync.size) {
            ...
            this.addEventListener('context-request', event => {
                ...
                if (!provides.has(name) && !providesAsync.has(name))
                    return;
                const value = this[name];
                const dispose = () => instanceContexts.get(name)?.delete(callback);
                const eventCallback = event.callback;
                let callback = (newValue) => eventCallback(newValue, dispose);
                if (providesAsync.has(name)) {
                    callback = async (newValue) => eventCallback(await newValue, dispose);
                }
                if (event.multiple) {
